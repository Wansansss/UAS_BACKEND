...
$("#name").change(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            var x = '<input readonly class="form-control py-4" name="email" value="'+data.customer.email+'" type="text"/>';
            $("#email").append(x);

            $("#company").html('<label class="small mb-1" for="inputFirstName">Customer company</label>');
            var x = '<input readonly class="form-control py-4" name="company" value="'+data.customer.company+'" type="text"/>';
            $("#company").append(x);

            $("#phone").html('<label class="small mb-1" for="inputFirstName">Customer Phone</label>');
            var x = '<input readonly class="form-control py-4" name="phone" value="'+data.customer.phone+'" type="text"/>';
            $("#phone").append(x);

            $("#address").html('<label class="small mb-1" for="inputFirstName">Customer Address</label>');
            var x = '<input readonly class="form-control py-4" name="address" value="'+data.customer.address+'" type="text"/>';
            $("#address").append(x);
        }
    });
});
...
{{-- <script>
    ...
    $("#code").change(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log(data);
                $("#productName").html('<label class="small mb-1" for="inputFirstName">Product Name</label>');
                var x = '<input readonly class="form-control py-4" name="name" value="'+ data.product.name +'" type="text"/>';
